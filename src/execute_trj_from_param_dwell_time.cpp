...
int main(int argc, char **argv)
{
  ...
  ros::init(argc, argv, "dwell_time");
  ros::NodeHandle nh;
  ros::AsyncSpinner spinner(1);
  spinner.start();

  ros::ServiceClient m_srv_client_start=nh.serviceClient<configuration_msgs::StartConfiguration>("/configuration_manager/start_configuration");
  ...
  std::vector<double> time_between_repetion;
  if (!nh.getParam("/time_between_repetion", time_between_repetion))
  {
    time_between_repetion.push_back(0);
  }
  ...
  m_srv_start.request.start_configuration = "PI_PPC";
  ...
  m_srv_start.request.strictness = 1;
  for (int idt=1;idt<time_between_repetion.size();idt++)
  {
    double dwell_time=time_between_repetion.at(idt);
    m_srv_start.request.start_configuration = "PI_PPC";
    m_srv_client_start.call(m_srv_start);
    ros::Duration(1).sleep();

    for (int rep=0;rep<repetions;rep++)
    {

      ROS_INFO("repetion %d of %d, dwell time = %f",rep,repetions,time_between_repetion.at(idt-1));
      for (const std::string & current_trj_name : executed_trjs)
      {
        ...
        if (save_time == true)
        {
          log_name = current_trj_name + "_" + std::to_string(t0.sec);
        }

        nh.setParam("/binary_logger/test_name", log_name +"_dwell"+std::to_string(time_between_repetion.at(idt-1))+ "_rep" + std::to_string(rep));
    ...
    m_srv_start.request.start_configuration = "STOP";
    m_srv_client_start.call(m_srv_start);

    ros::Time tsleep=ros::Time::now();
    ROS_INFO("waiting %f second (%f minutes)",dwell_time*60,dwell_time);
    while (ros::ok() && ((ros::Time::now()-tsleep).toSec()<dwell_time*60))
    {
      ROS_INFO_THROTTLE(30,"wait %f minutes of %f",(ros::Time::now()-tsleep).toSec()/60, dwell_time);
      ros::Duration(1).sleep();
    }
  }
